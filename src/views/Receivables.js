...
class Payables extends React.Component{
    ...
    handleFilter = (filter) => {
            ...
            const config = {
                ...
                if (filter !== "all") {
                   url = `${models.URL}/demands/${this.state.shopID}?page[size]=10sort[clear_date]=asc&filter[status]=${filter}`
                } else {
                    url = `${models.URL}/demands/${this.state.shopID}?page[size]=10sort[clear_date]=asc`
                }
    ...
    handleUpdateEquity = (equity) => {
        this.props.history.push(`/update-receiveable/${equity.id}`)
    }
