...
class UpdateReceivable extends React.Component{
    constructor(props){
        super(props)
        this.state = {
                year : "",
    ...
    handleClear = ()=>{
        ...
        axios.put(`${models.URL}/demands/${this.props.match.params.id}`, null,config)
            .then(response => {
    ...
    handleSubmit = ()=>{
        ...
        const config = {
            ...
            axios.post(`${models.URL}/demands`, JSON.stringify(body), config)
            .then(response => {
                this.props.onAlert(true,false,"با موفقیت ثبت شد")
            }).catch(err =>{
                this.props.onAlert(true,true,err.message)
    ...
    render(){
        return (
            ...
            <Alert show={this.props.alert.show} message={this.props.alert.message} error={this.props.alert.error}/>
                <CCard >
                    <CCardHeader>
                        <CButton >بروز رسانی حساب دریافتی</CButton>
                    </CCardHeader>
                    <CCardBody>
                    <CRow className="mb-3">
                        <CCol xs="12" md="6" className="mb-2">
                            <CLabel>مبلغ دریافتی</CLabel>
...
export default connect(mapStateToProps, mapDispatchToProps)(UpdateReceivable)